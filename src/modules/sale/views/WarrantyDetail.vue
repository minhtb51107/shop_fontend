...
              </select>
              <button class="btn btn-primary" @click="updateWarrantyStatus" :disabled="isUpdating">
...
          </div>
        </div>
        <div v-if="['RESOLVED', 'CANCELLED'].includes(warrantyCase.status)" class="alert alert-info mt-3 p-2">
...
const error = ref('');
const newStatus = ref('');
const isUpdating = ref(false); // THÊM: Biến kiểm soát trạng thái cập nhật
...
const updateWarrantyStatus = async () => {
    if (!confirm(`Bạn có chắc muốn chuyển trạng thái sang "${getStatusText(newStatus.value)}"?`)) return;
